...
void search(Contact *contacts, int i)
{
	int			search_index;
	...
	do
	{
		...
		index_ok = true;
		if (index_str.length() == 0 || index_str.find_first_not_of("0123456789") != std::string::npos)
		{
			std::cout << "Erreur, l'entree n'est pas un nombre au";
			std::cout << " format valide (pas de + ou de -)" << std::endl;
			index_ok = false;
		}
